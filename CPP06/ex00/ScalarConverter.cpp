...
bool check_float(std::string s) {
  ...
  std::string science_stuff[3] = {"-inff", "+inff", "nanf"};
  for (int j = 0; j < 3; j++) {
...
void ScalarConverter::convert(std::string s) {
  ...
  if (check_char(s) == true) {
    ...
    std::cout << "either a literal ABzc... or number 42/42.25f/52.25"
              << std::endl;
    return;
  }
  print_all(d);
...
void print_float(double d) {
  ...
  bool is_inf = (f == std::numeric_limits<float>::infinity() ||
...
void print_double(double d) {
  bool is_inf = (d == std::numeric_limits<double>::infinity() ||
