...
int Span::shortestSpan() {
  ...
  for (std::vector<int>::iterator i = tmp.begin() + 1; i != tmp.end() - 1;
       i++) {
    if (*(i + 1) - *i < shortest)
      shortest = *(i + 1) - *i;
  }
...
unsigned int Span::get_arr_size() const 
{ return (arr_size); } 
unsigned int Span::get_arr_length() const 
{ return (arr_length); }
